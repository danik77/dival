...
const INITIAL_ERRORS = ["name", "email", "phone", "country", "city", "category"]
...
*/
		...
		fetch("https://countriesnow.space/api/v0.1/countries")
			...
			.then((result) => {
				...
				countriesData.map((item) => {
					countriesOptions.push({ label: item.country, value: item.country });
				});
  ...
  */
	...
	useEffect(() => {
	 ...
	 apiData.length &&  axios.get('https://get.geojs.io/v1/ip/geo.js').then(res => {
const detectCountry = JSON.parse(res.data.slice(6, -2)).country
setFormData({...formData, country: {label: detectCountry, value: detectCountry}})
	...
	const handleChange = (e: any) => {
		...
		if (e.target.value !== "" && e.target.name !== "email") {
			...
			const arr = validationError.filter((item) => item !== e.target.name);
			console.log(arr);
			setValidationError(arr);
		}

		if (e.target.value === "" && !validationError.includes(e.target.name) && e.target.name !== "email") {
		...
		if(e.target.name === "email"){
			const re =  /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
				...
				if(e.target.value.toLowerCase().match(re) || e.target.value === "") {
						setValidEmail(true)
				} else {
					setValidEmail(false)
				}
	...
	// 	fillCities(selectedCountry)
		...
		const country = apiData.find((item) => item.country === selectedCountry);
...
const sendMail = await axios.post('/api/contact', {data: formData, email: context.contacts.data.attributes.Email});
							...
							<div className={style.contact__header}>
								...
								<>
									<input
										...
										placeholder={t("name") + "*"}
										required={true}
										className={validationError.includes("name") ? style.contact__errorInput : style.contact__input}
									/>
									<div
...
{!validEmail &&
									...
									{context.categories && (
										<>
										<h4 className={style.contact__categoriesTitle}>{t('what-is-your-project')}</h4>
										<div
												className={style.contact__helperText}
												style={{
													display: validationError.includes("category")
														? "block"
														: "none",
												marginTop: "2px",
    									marginBottom: "0px"
											...
											<div className={style.contact__categories}>
												{context.categories.map((category) => (
													<div className={style.contact__category}>
														<input
															type="radio"
															id={category.id}
															name="category"
														...
														<label htmlFor={category.id}>
															{router.locale === "uk"
																...
																: category.attributes.localizations.data.find(
																		(item: any) =>
																			item.attributes.locale === router.locale
																  ).attributes.Name}
														</label>
													</div>
												))}
												...
												<div className={style.contact__category}>
													<input
														...
														onChange={handleChange}
													/>
													<label htmlFor="categoryOther">{t("other")}</label>
												</div>
											</div>
									...
									<div className={style.contact__footer}>
										...
										<button
											...
											disabled={!!validationError.length || !validEmail}
										>
											{sending ?  t("sending") : t("get-consultation")}
										</button>
									</div>
								</>
		 	...
		 	{sent && (
									...
									<ContactSent closePopup={closePopup} type={props.type} />
